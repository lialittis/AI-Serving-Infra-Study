...
def compare(eager, graph):
    ...
    require({k:v for k,v in envs[0].items() if k!='P12_TRACE_DIR'} ==
            {k:v for k,v in envs[1].items() if k!='P12_TRACE_DIR'}, 'environment overrides differ')
    ...
    return dict(runs=rows,matching_requests=True,matching_sources=True,matching_instrumentation=True,
