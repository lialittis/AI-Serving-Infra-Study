...
def analyze(run, records=None, trace=None):
    ...
    command = load(run / 'command.json')['argv']
    require('--enforce-eager' in command and command[command.index('--num-gpu-blocks-override')+1] == '2',
            'small eager pool configuration missing')
    ...
    require(not links.replay_coverage(), 'unexpected graph replay')
    first_b=min((c['kernel'] for c in chains if c['role']=='B'),key=lambda k:number(k['ts']))
    last_a=a['steps'][-1]['last_kv_access']
    result=dict(requests=requests, reused_block=1, layers=24, verified_kv_operator_chains=len(chains),
                ...
                limits=['serial eager requests, prefix caching and async scheduling disabled',
