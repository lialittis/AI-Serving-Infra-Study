...
def render(run):
    ...
    add('A decode 原生 Event 等待返回',end(s['native_wait']),'A','本次 event 和 KV kernel 在 CPU 进入等待前已完成；不能把等待耗时解释为 KV kernel 耗时。')
    ...
    svg.append('<text x="20" y="198" font-family="sans-serif" font-size="13" fill="#426166">真实 allocator + 真实 NPU 任务；串行 eager 基线。没有人工 generation 字段，也没有额外插入设备等待。</text></svg>')
...
<div class="grid"><div class="card"><h2>关联依据</h2><p id="counts"></p><p>每条 KV/FIA 链均核对 async_npu、HostToDevice、CANN connection ID 和计算 kernel CSV；结果传输另核对 MEMCPY_ASYNC 与 EVENT_RECORD。</p><p>CPU 等待来自原生 _to_list 中的 transfer_event.synchronize()。观测脚本没有增加设备同步或数据读回。</p></div><div class="card"><h2>明确保留的边界</h2><ul><li>KV 访问是按算子语义、参数和 layer 关联，并非设备内存指令追踪。</li><li>本次 event 在 CPU 开始等待前已完成；不能将等待耗时当作 KV 计算耗时。</li><li>block free 不等于物理清零；没有读取旧数据来检查清零或逐元素覆盖。</li><li>prefix sharing、并发请求、图重放、async scheduling 尚未在这里验证。</li></ul></div></div>
