...
def collect_sources(output, model):
    files = {
        "vllm": ["vllm/config/cache.py", "vllm/v1/core/block_pool.py",
                 ...
                 "vllm/v1/utils.py", "vllm/compilation/backends.py", "vllm/compilation/piecewise_backend.py"],
    ...
    import sysconfig
    for name in ('graphs.py','streams.py'):
        source=Path(sysconfig.get_paths()['purelib'])/'torch_npu/npu'/name
        target=output/'sources/torch_npu/npu'/name
        target.parent.mkdir(parents=True,exist_ok=True)
        target.write_bytes(source.read_bytes())
        manifest.setdefault('torch_npu',{'files':{}})['files']['npu/'+name]=hashlib.sha256(source.read_bytes()).hexdigest()
...
def main():
    ...
    snapshot.mkdir()
    hashes = {}
    for name in ("sitecustomize.py", "lifetime_trace.py", "resource_observer.py", "run_reuse.py"):
    ...
    overrides = dict(HF_HUB_OFFLINE="1", TRANSFORMERS_OFFLINE="1",
                     VLLM_WORKER_MULTIPROC_METHOD="spawn", P12_MODE=args.mode, P12_TRACE_DIR=str(output / "events"),
