"""Start an isolated local API service, send one request, archive and stop it."""
...
def main():
    ...
    snapshot.mkdir()
    hashes = {}
    for name in ("sitecustomize.py", "trace_hooks.py", "run_trace.py", "summarize_trace.py"):
    ...
    command = [sys.executable, "-m", "vllm.entrypoints.cli.main", "serve", args.model,
               "--served-model-name", "qwen-trace", "--host", "127.0.0.1", "--port", str(args.port),
    ...
    env.update(HF_HUB_OFFLINE="1", TRANSFORMERS_OFFLINE="1",
               VLLM_WORKER_MULTIPROC_METHOD="spawn", P07_TRACE_DIR=str(output / "events"),
               PYTHONPATH=str(root) + os.pathsep + env.get("PYTHONPATH", ""))
    save(output / "command.json", {"argv": command,
         "environment_overrides": {key: env[key] for key in (
             "HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE", "VLLM_WORKER_MULTIPROC_METHOD", "P07_TRACE_DIR", "PYTHONPATH")}})
    request_body = {"model": "qwen-trace", "prompt": "The capital of France is",
                    "max_tokens": 8, "temperature": 0, "ignore_eos": True,
                    "stream": False, "seed": 0, "request_id": "practice07-single-request"}
    ...
    subprocess.run([sys.executable, str(root / "summarize_trace.py"), str(output)], check=True)
