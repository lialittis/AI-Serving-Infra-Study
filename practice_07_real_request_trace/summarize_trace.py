...
def summarize(directory):
    ...
    if by_kind["api_response"][0]["response"] != response:
        raise ValueError("API trace response differs from the HTTP response")
    ...
    if len(schedule) != request["max_tokens"]:
        raise ValueError("unexpected number of steps for the synchronous baseline")
    computed = 0
    for index, event in enumerate(schedule):
        expected = response["usage"]["prompt_tokens"] if index == 0 else 1
        if (event["step"] != index + 1 or event["scheduled_tokens"] != {rid: expected}
                or event["before"][rid]["computed_tokens"] != computed):
            raise ValueError("token progression differs from the synchronous baseline")
        computed += expected
    for kind in ("worker_call", "runner_call", "model_forward", "attention_first_layer", "engine_output"):
        if len(by_kind[kind]) != len(schedule):
            raise ValueError(f"{kind} does not cover every scheduled step")
        if [row.get("step") for row in by_kind[kind]] != [row["step"] for row in schedule]:
            raise ValueError(f"{kind} step correlation unavailable for this executor")
    lines = ["# Practice 07：实际请求追踪摘要", "", f"- Engine request ID：`{rid}`",
             ...
             f"- Scheduler 对象：`{config['scheduler']}`",
