"""Profile one real vLLM request: Python scopes, torch operators and NPU kernels."""
...
def main():
    ...
    snapshot.mkdir()
    hashes = {}
    for name in ("sitecustomize.py", "operator_trace.py", "run_profile.py", "summarize_profile.py"):
    ...
    command = [sys.executable, "-m", "vllm.entrypoints.cli.main", "serve", args.model,
               "--served-model-name", "qwen-op-trace", "--host", "127.0.0.1", "--port", str(args.port),
    ...
    env = os.environ.copy()
    for key in ("P07_TRACE_DIR", "P08_TRACE_DIR"):
        env.pop(key, None)
    env.update(HF_HUB_OFFLINE="1", TRANSFORMERS_OFFLINE="1",
               VLLM_WORKER_MULTIPROC_METHOD="spawn", P09_TRACE_DIR=str(output / "events"),
    ...
    save(output / "command.json", {"argv": command,
         "environment_overrides": {key: env[key] for key in (
             "HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE", "VLLM_WORKER_MULTIPROC_METHOD", "P09_TRACE_DIR", "PYTHONPATH")}})
    ...
    request_body = {"model": "qwen-op-trace", "prompt": prompt_ids,
                    "max_tokens": 2, "temperature": 0, "ignore_eos": True,
                    "stream": False, "seed": 0, "request_id": "practice09-profile"}
    ...
    with (output / "server.log").open("w") as log:
        ...
        try:
            ...
            warmup = dict(request_body, request_id="practice09-warmup")
    ...
    subprocess.run([sys.executable, str(root / "summarize_profile.py"), str(output)], check=True)
